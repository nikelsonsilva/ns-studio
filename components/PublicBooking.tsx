...
const PublicBooking: React.FC<PublicBookingProps> = ({ onBack, businessSlug }) => {
  ...
  const [businessData, setBusinessData] = useState<{
   ...
   city: string | null;
   state: string | null;
  }>({ name: '', logo_url: null, address: null, complement: null, phone: null, business_hours: null, zip_code: null, city: null, state: null });
     ...
     if (businessSlug) {
      ...
      if (id) {
        ...
        const { data } = await supabase
         .from('businesses')
         .select('business_name, name, logo_url, address, complement, phone, business_hours, zip_code, city, state')
         .eq('id', id)
         .single();
        if (data) {
         setBusinessData({
     ...
     try {
      ...
      for (let hour = earliestStart; hour < latestEnd; hour++) {
        ...
        const hasAvailableProf = workingProfs.some(wp => {
         ...
         const dateStr = format(selectedDateObj, 'yyyy-MM-dd');
         const slotDateTime = new Date(`${dateStr}T${hour.toString().padStart(2, '0')}:00:00`);
  ...
  }, [businessId, selectedDateObj, selectedServices.length, step, profAvailability.length, dayAppointments.length, timeBlocks.length, Object.keys(professionalServiceMap).length]);
