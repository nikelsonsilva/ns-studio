...
/**
 * Endpoint para proxy do Abacate Pay (evitar CORS)
 */
app.post(
    '/api/abacatepay/billing',
    express.json(),
    async (req, res) => {
        console.log('🥑 [AbacatePay Proxy] Creating billing...');

        try {
            const { apiKey, billingData } = req.body;

            if (!apiKey) {
                return res.status(400).json({ error: 'API Key is required' });
            }

            console.log('🥑 [AbacatePay Proxy] Request body:', JSON.stringify(billingData, null, 2));
            ...
            if (!response.ok) {
                console.error('🥑 [AbacatePay Proxy] API Error:', data);
                return res.status(response.status).json(data);
            }

            console.log('🥑 [AbacatePay Proxy] Success:', data);
            res.json(data);
        } catch (error: any) {
